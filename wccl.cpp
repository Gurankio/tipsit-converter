...
string toWBase(string value, string encoding, bool spaces = true) {
  ...
  for (int i = 0; i < value.length(); ++i) {
    if (value[i] != '.') {
      if (encoding == "Aiken") res += aiken(value[i]);
      else if (encoding == "BCD") res += bcd(value[i]);
      else if (encoding == "Quinario") res += quinary(value[i]);
      else if (encoding == "Biquinario") res += biquinary(value[i]);
      else if (encoding == "2su5") res += twoOutOfFive(value[i]);
    } else res += ".";

    // adding spaces
    if (spaces && value[i + 1] != '.' && value[i + 1] != '\0' && value[i] != '.') res += " ";
  }
...
string fromWBase(string value, string encoding, bool spaces = true) {
  ...
  if (encoding == "Aiken") weights = { 2, 4, 2, 1 };
  else if (encoding == "BCD") weights = { 8, 4, 2, 1 };
  else if (encoding == "Quinario") weights = { 5, 4, 2, 1 };
  else if (encoding == "Biquinario") {
    weights = { 0, 5, 0, 4, 3, 2, 1, 0 };

    if (!spaces) weights = { 0, 5, 4, 3, 2, 1, 0 };
